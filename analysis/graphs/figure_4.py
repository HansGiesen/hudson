...
query = 'select program.name as benchmark, platform.name as platform, seed, args, proc_freq,' \
        ' tuning_run.start_date, collection_date, fidelity, result.state, run_time, test.name as test from result' \
...
data.loc[data['technique'] == 'Bayes', 'max_fidelity'] = 1
...
data = data.loc[data.groupby(['test', 'seed', 'time'])['run_time'].idxmin()]

# Compute the shortest runtime so far.
data['run_time'] = data.groupby(['test', 'seed'])['run_time'].cummin()

# Use a common time series for each experiment with the same seeds.
data = data.groupby('test').apply(resample).reset_index(drop=True)

# Compute the average and standard deviation of the average.
groups = data.groupby(['benchmark', 'platform', 'test', 'time'])
...
descriptions = {'no_error': 'Without build error model', 'bayes_zcu102': 'With build error model'}

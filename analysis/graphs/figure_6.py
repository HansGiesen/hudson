...
def mean_std(values):
    std_dev = np.std(values) / math.sqrt(len(values))
...
query = 'select program.name as benchmark, platform.name as platform, seed, args, proc_freq,' \
...
# Get the number of initialization seeds of each tuning run.
data['seed_cnt'] = args.transform(lambda field: getattr(field, 'bayes_seed_cnt', 0))
...
data['run_time'] = data.groupby(['benchmark', 'platform', 'technique', 'seed', 'seed_cnt'])['run_time'].cummin()

# Use a common time series for each experiment with the same seeds.
data = data.groupby(['benchmark', 'platform', 'technique', 'seed_cnt']).apply(resample).reset_index(drop=True)

# Compute the average and standard deviation of the average.
groups = data.groupby(['benchmark', 'platform', 'time', 'technique', 'seed_cnt'])
...
techniques = {'AUCBanditMetaTechniqueA': 'Multi-armed bandit', 'MultiFidBayes': 'Bayesian optimization'}
plots = []
names = []
groups = data.groupby(['benchmark', 'platform', 'technique', 'seed_cnt', 'seeds'])
for (benchmark, platform, technique, seed_cnt, seeds), group in groups:
    plot_line = plt.step(group['time'], group['mean'], where='post')[0]
    plot_fill = plt.fill_between(group['time'], group['min'], group['max'], step='post', alpha=0.2)
    plots.append((plot_line, plot_fill))
    names.append(str(seed_cnt))
plt.xlim([0.4, 2.4])
#plt.ylim([0, None])
plt.ylim([0.1, 2.0])
plt.yscale('log')

...
query = 'select program.name as benchmark, platform.name as platform, seed, args, proc_freq, tuning_run.name,' \
        ...
        ' where test.name = "pipe_zcu102" and tuning_run.name like "bnn_%%" or' \
...
# Remove the seed from the tuning run names.
...
data = data.loc[data.groupby(['benchmark', 'platform', 'seed', 'name', 'time'])['run_time'].idxmin()]

# Compute the shortest runtime so far.
data['run_time'] = data.groupby(['benchmark', 'platform', 'seed', 'name'])['run_time'].cummin()

# Use a common time series for each experiment with the same seeds.
data = data.groupby(['benchmark', 'platform', 'name']).apply(resample).reset_index(drop=True)

# Compute the average and standard deviation of the average.
groups = data.groupby(['benchmark', 'platform', 'time', 'name'])
...
techniques = {'AUCBanditMetaTechniqueA': 'Multi-armed bandit', 'PipelinedMultiFidBayes': 'Bayesian optimization',
              'Random': 'Random search'}
descriptions = {'bnn': 'SF BO',
                'bnn_1x1.2x2.3x1': '1x1,2x2,3x1 MF BO',
                'bnn_2x1.2x2.2x1': '2x1,2x2,2x1 MF BO',
                'bnn_2x1.3x1.3x1': '2x1,3x1,3x1 MF BO',
                'bnn_4x1.1x2.1x2': '4x1,1x2,1x2 MF BO'}

...
void projection ( Triangle_3D triangle_3d, Triangle_2D *triangle_2d, int angle )
{
    ...
    printf("Projection input: %u %u %u %u %u %u %u %u %u\n",
           triangle_3d.x0, triangle_3d.y0,
           triangle_3d.z0, triangle_3d.x1,
           triangle_3d.y1, triangle_3d.z1,
           triangle_3d.x2, triangle_3d.y2,
           triangle_3d.z2);
  #endif
  ...
  else if(angle == 2)
  {
    ...
    printf("Projection output: %u %u %u %u %u %u %u\n",
           triangle_2d->x0, triangle_2d->y0,
           triangle_2d->x1, triangle_2d->y1,
           triangle_2d->x2, triangle_2d->y2,
           triangle_2d->z);
  #endif
}
...
bool rasterization1 ( Triangle_2D & triangle_2d, bit8 max_min[], int max_index[])
{
  ...
  max_min[4] = max_min[1] - max_min[0] + 1;

  // calculate index for searching pixels
  max_index[0] = (max_min[1] - max_min[0] + 1) * (max_min[3] - max_min[2] + 1);
    ...
    if( pixel_in_triangle( x, y, triangle_2d ) )
    {
      ...
      fragment[i].color = color;
        ...
        printf("Rasterization 2 output %u: %u %u %u %u\n", i,
               fragment[i].x, fragment[i].y, fragment[i].z,
               fragment[i].color);
      #endif
      i++;
    }
...
int zculling ( int counter, CandidatePixel fragments[], int size, Pixel pixels[])
{
  ...
  {
    if( fragments[n].z < z_buffer[fragments[n].x][fragments[n].y] )
    {
      pixels[pixel_cntr].x     = fragments[n].x;
      pixels[pixel_cntr].y     = fragments[n].y;
      pixels[pixel_cntr].color = fragments[n].color;
      z_buffer[fragments[n].x][fragments[n].y] = fragments[n].z;
      #ifdef DEBUG
        printf("Z-culling output %u: %u %u %u\n", pixel_cntr,
               pixels[pixel_cntr].x, pixels[pixel_cntr].y,
               pixels[pixel_cntr].color);
      #endif
      pixel_cntr++;
    }
  }

...
void binarize_input_images(Word* dmem_i, const float* inputs, unsigned S,
  ...
  const unsigned C = 3;
  const unsigned W = C1InputType::width;
  const int factor = 1 << (W - C1InputType::iwidth);
  const int mask = (1 << W) - 1;
  assert(W <= WORD_SIZE/C);
  unsigned addr_o = 0;
  unsigned img_addr = 0;
  for (unsigned img = 0; img < batch_size; ++img) {
    for (unsigned s = 0; s < S*S; ++s) {
      unsigned addr_i = img_addr + s;
      uint64_t wrd = 0;
      for (unsigned c = 0; c < C; ++c) {
        int64_t t1 = floor(inputs[addr_i] * factor + 0.5);
        wrd |= (t1 & mask) << (W * c);
        addr_i += S * S;
      }
      dmem_i[addr_o++] = wrd;

...
int main(int argc, char** argv) {
  ...
  const unsigned kh_size = layer_is_last(l) ? 2 * N / KH_PER_WORD : N/KH_PER_WORD;

  Word* wt      = new Word[wt_size];
  Word* kh      = new Word[kh_size];
  Word* data_i  = (Word*) MEM_ALLOC( DMEM_WORDS * sizeof(Word) );
  Word* data_o  = (Word*) MEM_ALLOC( (layer_is_last(l) ? 1 : (N*So*So/WORD_SIZE)) * sizeof(Word) );
  ...
  if (l == 1) {
    ...
    binarize_input_images(data_i, X.data, Si, 1);
  } else {

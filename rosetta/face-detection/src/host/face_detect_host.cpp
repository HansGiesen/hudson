...
int main(int argc, char ** argv) 
{
  ...
  // sdsoc host code
    ...
    #else
      unsigned char (* input)[IMAGE_WIDTH];
      input = (unsigned char (*)[IMAGE_WIDTH]) alloc_input(IMAGE_HEIGHT * IMAGE_WIDTH);
      for ( int i = 0; i < IMAGE_HEIGHT; i ++ )
        for ( int j = 0; j < IMAGE_WIDTH; j ++ )
          input[i][j] = Data[i][j];

      for ( int i = 0; i < IMAGE_HEIGHT-1; i ++ )
        face_detect(input[i], result_x, result_y, result_w, result_h, &res_size);
    #endif
    ...
    #else
      face_detect(input[IMAGE_HEIGHT-1], result_x, result_y, result_w, result_h, &res_size);
    #endif
    #ifdef __SDSCC__
  ...
  bool error = check_results(res_size, result_x, result_y, result_w, result_h, Data, outFile);

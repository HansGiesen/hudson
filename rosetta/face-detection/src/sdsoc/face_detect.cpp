...
void classifyImage      ( float _factor,
                          int sum_row,
                          int sum_col,
                          int *AllCandidates_x,
                          int *AllCandidates_y,
...
{
  static unsigned char Data[PIPELINE_CNT][IMAGE_HEIGHT][IMAGE_WIDTH];
  ...
  if ( counter < IMAGE_HEIGHT){
    ...
    for( j = 0; j < IMAGE_WIDTH; j++){
      for ( k = 0; k < PIPELINE_CNT; k++) {
        #pragma HLS unroll 
        Data[k][counter][j] = inData[j];
      }
    }
    counter++;
    if ( counter < IMAGE_HEIGHT ){
  ...
  *result_size = 0;
  ...
  int result_x_Scale[PIPELINE_CNT][RESULT_SIZE];
  #pragma HLS array_partition variable=result_x_Scale complete dim=1
  int result_y_Scale[PIPELINE_CNT][RESULT_SIZE];
  #pragma HLS array_partition variable=result_y_Scale complete dim=1
  int result_w_Scale[PIPELINE_CNT][RESULT_SIZE];
  #pragma HLS array_partition variable=result_w_Scale complete dim=1
  int result_h_Scale[PIPELINE_CNT][RESULT_SIZE];
  #pragma HLS array_partition variable=result_h_Scale complete dim=1
  int res_size_Scale[PIPELINE_CNT];
  #pragma HLS array_partition variable=res_size_Scale complete dim=1
  ...
  for ( i = 0; i < PIPELINE_CNT; i++){
    Output_inner:
    for ( j = 0; j < res_size_Scale[i]; j++){
      #pragma HLS loop_tripcount max=43
      result_x[k] = result_x_Scale[i][j];
      result_y[k] = result_y_Scale[i][j];
      result_w[k] = result_w_Scale[i][j];
      result_h[k] = result_h_Scale[i][j];
      k++;
    }
  }
  *result_size = k;
...
{
  float factors[] = {1.2, 1.44, 1.728, 2.0736, 2.48832, 2.98598, 3.58318, 4.29981,
                     5.15978, 6.19173, 7.43008, 8.9161};

  int winWidths[]  = {29, 35, 41, 50, 60, 72, 86, 103, 124, 149, 178, 214};
  int winHeights[] = {29, 35, 41, 50, 60, 72, 86, 103, 124, 149, 178, 214};

  int imgWidths[]  = {266, 222, 185, 154, 128, 107, 89, 74, 62, 51, 43, 35};
  int imgHeights[] = {199, 166, 138, 115,  96,  80, 66, 55, 46, 38, 32, 26};

  res_size_Scale = 0;

  Process_inner:
  for ( int j = 0; j < pipeline_len[pipeline]; j++)
  {
    const unsigned avg_win_cnt = int(12.0 / PIPELINE_CNT);
    #pragma HLS loop_tripcount max=avg_win_cnt

    unsigned idx = pipeline_idx[pipeline][j];

    float factor = factors[idx];

    /* size of the image scaled up */
    MySize winSize = { winWidths[idx], winHeights[idx] };
    /* size of the image scaled down (from bigger to smaller) */
    MySize sz = { imgWidths[idx], imgHeights[idx] };
...
)
{
  ...
  Pixely: for( y = 0; y < sum_row; y++ ){
    #pragma HLS loop_tripcount max=121
    Pixelx : for ( x = 0; x < sum_col; x++ ){
      #pragma HLS loop_tripcount max=118

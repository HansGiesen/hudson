...
int main(int argc, char *argv[]) 
{
  ...
  // sdsoc version host code
    ...
    #else
      VectorDataType* data_points_for_accel = (VectorDataType*)alloc_data(sizeof(VectorDataType) * NUM_TRAINING * NUM_FEATURES / D_VECTOR_SIZE);
      VectorLabelType* labels_for_accel = (VectorLabelType*)alloc_label(sizeof(VectorLabelType) * NUM_TRAINING / L_VECTOR_SIZE);
      VectorFeatureType* param_for_accel = (VectorFeatureType*)alloc_theta(sizeof(VectorFeatureType) * NUM_FEATURES / F_VECTOR_SIZE);
    #endif
  ...
  double error_rate = check_results( param_vector, data_points, labels );
  ...
  // cleanup
    ...
    #else
      free_data(data_points_for_accel);
      free_label(labels_for_accel);
      free_theta(param_for_accel);
    #endif
  #endif
